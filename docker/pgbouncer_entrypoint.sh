...
if [ -n "$DB_USER1" -a -n "$DB_PASSWORD1" -a -e "${_AUTH_FILE}" ] && ! grep -q "^\"$DB_USER1\"" "${_AUTH_FILE}"; then
  if [ "$AUTH_TYPE" == "plain" ] || [ "$AUTH_TYPE" == "scram-sha-256" ]; then
     pass="$DB_PASSWORD1"
  else
     pass="md5$(echo -n "$DB_PASSWORD1$DB_USER1" | md5sum | cut -f 1 -d ' ')"
  fi
  echo "\"$DB_USER1\" \"$pass\"" >> ${PG_CONFIG_DIR}/userlist.txt
  echo "Wrote authentication credentials to ${PG_CONFIG_DIR}/userlist.txt"
fi

if [ -n "$DB_USER2" -a -n "$DB_PASSWORD2" -a -e "${_AUTH_FILE}" ] && ! grep -q "^\"$DB_USER2\"" "${_AUTH_FILE}"; then
  if [ "$AUTH_TYPE" == "plain" ] || [ "$AUTH_TYPE" == "scram-sha-256" ]; then
     pass="$DB_PASSWORD2"
  else
     pass="md5$(echo -n "$DB_PASSWORD2$DB_USER2" | md5sum | cut -f 1 -d ' ')"
  fi
  echo "\"$DB_USER2\" \"$pass\"" >> ${PG_CONFIG_DIR}/userlist.txt
  echo "Wrote authentication credentials to ${PG_CONFIG_DIR}/userlist.txt"
fi

if [ -n "$DB_USER3" -a -n "$DB_PASSWORD3" -a -e "${_AUTH_FILE}" ] && ! grep -q "^\"$DB_USER3\"" "${_AUTH_FILE}"; then
  if [ "$AUTH_TYPE" == "plain" ] || [ "$AUTH_TYPE" == "scram-sha-256" ]; then
     pass="$DB_PASSWORD3"
  else
     pass="md5$(echo -n "$DB_PASSWORD3$DB_USER3" | md5sum | cut -f 1 -d ' ')"
  fi
  echo "\"$DB_USER3\" \"$pass\"" >> ${PG_CONFIG_DIR}/userlist.txt
  echo "Wrote authentication credentials to ${PG_CONFIG_DIR}/userlist.txt"
fi
...
################## Auto generated ##################
[databases]
${DB_NAME1:-*}=host=${DB_HOST1:?"Setup pgbouncer config error! You must set DB_HOST env"} port=${DB_PORT1:-5432} user=${DB_USER1:-postgres}
${DB_NAME2:-*}=host=${DB_HOST2:?"Setup pgbouncer config error! You must set DB_HOST env"} port=${DB_PORT2:-5432} user=${DB_USER2:-postgres}
${DB_NAME3:-*}=host=${DB_HOST3:?"Setup pgbouncer config error! You must set DB_HOST env"} port=${DB_PORT3:-5432} user=${DB_USER3:-postgres}
